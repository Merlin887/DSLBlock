...
def need_ortog(epoch, argdict) -> bool:
	...
	if argdict['orthog_epochs'] > 0 and epoch <= argdict['epochs'] * 0.75:
		return True
...
def binary_lr_scheduler(epoch, argdict):
	initial_learning_rate = argdict['lr']
	minimal_rate = 0.000001
	k = epoch // 10
	calculated_rate = initial_learning_rate * (0.5 ** k)
	return max(calculated_rate, minimal_rate)
